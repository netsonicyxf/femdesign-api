...
namespace FemDesign.Results
{
    ...
    public partial class BarInternalForce : IResult
    {
        ...
        internal static Regex IdentificationExpression
        {
            get
            {
                return new Regex(@"^(?'type'Bars), (?'result'Internal forces), ((?'loadcasetype'[\w\s\-]+)? - )?Load (?'casecomb'case|comb\.): (?'casename'[ -#%'-;=?A-\ufffd]{1,79}?)(?: - selected objects)?$");
            }
        }
        ...
        internal static Regex HeaderExpression
        {
            get
            {
                return new Regex(@"^(?'type'Bars), (?'result'Internal forces), ((?'loadcasetype'[\w\s\-]+)? - )?Load (?'casecomb'case|comb\.): (?'casename'[ -#%'-;=?A-\ufffd]{1,79}?)(?: - selected objects)?$|^ID\tx\tN\tTy'\tTz'\tMt,SV\tMy'\tMz'\tB\tMt,w\tMt,tot\t(Case|Comb\.)|^\[.+\]");
            }
        }
