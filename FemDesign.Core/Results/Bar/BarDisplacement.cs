...
namespace FemDesign.Results
{
    ...
    public partial class BarDisplacement : IResult
    {
        ...
        internal static Regex IdentificationExpression
        {
            get
            {
                return new Regex(@"^(?'type'Bars), (?'result'Displacements), ((?'loadcasetype'[\w\s\-]+)? - )?Load (?'casecomb'case|comb\.): (?'casename'[ -#%'-;=?A-\ufffd]{1,79}?)(?: - selected objects)?$");
            }
        }
        ...
        internal static Regex HeaderExpression
        {
            get
            {
                return new Regex(@"^(?'type'Bars), (?'result'Displacements), ((?'loadcasetype'[\w\s\-]+)? - )?Load (?'casecomb'case|comb\.): (?'casename'[ -#%'-;=?A-\ufffd]{1,79}?)(?: - selected objects)?$|^Bar\tx\tex'\tey'\tez'\t\u03c6 x'\t\u03c6 y'\t\u03c6 z'\t\u03D1\t(Case|Comb\.)|^\[.+\]");
            }
        }
